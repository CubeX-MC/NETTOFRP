...
| `internal/prober` | 多次并行采样采集延迟、抖动、成功率、中位延迟、带宽 |
| `internal/selector` | 加权综合评分与候选线路排序，含非线性延迟评分、中位延迟、恢复期惩罚、选路滞回、地理就近 + 评分综合权衡 |
...
- **快速探测**：每条线路的探测采样并行发出，整轮探测最坏耗时从「样本数 × 超时」降为单个超时，避免死线路拖慢整个探测循环
