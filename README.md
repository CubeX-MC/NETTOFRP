...
不同线路的后端服务器容量不同（如线路 A 支持 100 人，线路 B 只支持 50 人），只看绝对玩家数无法判断哪条更空闲。为此：

- 每条线路可配置 `max_load`（最大玩家容量，`0` 表示不限容量）
- 选择器维护**衰减计数器**估算各线路的实时在线人数：每次 Transfer 下发 +1，之后按 **30 分钟半衰期**指数衰减（模拟玩家陆续下线）
- 负载惩罚按**已用容量比例**而非绝对次数计算：`penalty = 1 - 0.15 × min(count/max_load, 1)`
  - 线路 A：容量 100，已用 80 → 惩罚 12%，余量 20
  - 线路 B：容量 50，已用 10 → 惩罚 1.5%，余量 40 → **B 更充裕，优先选择**
- 惩罚与延迟/稳定性评分相乘：**余量相似时选延迟低的，延迟相似时选余量大的**，二者自动权衡
- 未配置 `max_load` 的线路不参与负载惩罚（视为无限容量）
- 单调场景无副作用：所有线路均未满容量时惩罚轻微，不会因"都有玩家"而整体降分

```json
{ "name": "play1", "address": "play1.example.org:25565", "max_load": 100 },
{ "name": "play2", "address": "play2.example.org:25565", "max_load": 50 }
```
...
| `lines[].max_load` | 该线路后端服务器的最大玩家容量（`0` 表示不限容量）。设置后，选择器按衰减计数器估算在线人数，按已用容量比例施加评分惩罚，引导新玩家分流到更空闲的线路 |
...
- **容量感知负载均衡**：每条线路可配置 `max_load`，选择器用衰减计数器估算在线人数，按已用容量比例降权，余量大的线路优先
